  ...
  with tf.variable_scope(scope, 'MobilenetV1', [inputs], reuse=reuse) as scope:
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      with tf.variable_scope('Logits'):
        ...
        net = slim.dropout(net, 0.7, scope='dropout6',is_training=is_training)
        net = slim.conv2d(net, 256, [1, 1], activation_fn=None, normalizer_fn=None, scope='features')
        end_points['features'] = tf.squeeze(net, [1, 2], name='FeatureSqueeze')
        if not num_classes:
          return net, end_points
