...
def face_net_train(input_op, NUM_CLASSES, NUM_FEATURE,is_training):
    ...
    p = []
    # phase 1
    conv1 = conv_op(input_op, name="conv1", kh=3, kw=3, n_out=64, dh=1, dw=1, is_training=is_training, p=p)
    pool1 = mpool_op(conv1, name="pool1", kh=2, kw=2, dw=2,  dh=2)

    #phase 2
    conv2_1 = conv_op(pool1, name="conv2_1", kh=3, kw=3, n_out=96, dh=1, dw=1, is_training=is_training, p=p)
    conv2_2 = conv_op(conv2_1, name="conv2_2", kh=3, kw=3, n_out=128, dh=1, dw=1, is_training=is_training, p=p)
    pool2 = mpool_op(conv2_2, name="pool2", kh=2, kw=2, dw=2,  dh=2)

    # phase 3
    conv3_1 = conv_op(pool2, name="conv3_1", kh=3, kw=3, n_out=128, dh=1, dw=1, is_training=is_training, p=p)
    conv3_2 = conv_op(conv3_1, name="conv3_2", kh=3, kw=3, n_out=160, dh=1, dw=1, is_training=is_training, p=p)
    conv3_3 = conv_op(conv3_2, name="conv3_3", kh=3, kw=3, n_out=192, dh=1, dw=1, is_training=is_training, p=p)
    pool3 = mpool_op(conv3_3, name="pool3", kh=2, kw=2, dw=2,  dh=2)

    # phase 4
    conv4_1 = conv_op(pool3, name="conv4_1", kh=3, kw=3, n_out=192, dh=1, dw=1, is_training=is_training, p=p)
    conv4_2 = conv_op(conv4_1, name="conv4_2", kh=3, kw=3, n_out=224, dh=1, dw=1, is_training=is_training, p=p)
    conv4_3 = conv_op(conv4_2, name="conv4_3", kh=3, kw=3, n_out=256, dh=1, dw=1, is_training=is_training, p=p)
    pool4 = mpool_op(conv4_3, name="pool4", kh=2, kw=2, dw=2, dh=2)

    # phase 5
    conv5_1 = conv_op(pool4, name="conv5_1", kh=3, kw=3, n_out=256, dh=1, dw=1, is_training=is_training, p=p)
    conv5_2 = conv_op(conv5_1, name="conv5_2", kh=3, kw=3, n_out=256, dh=1, dw=1, is_training=is_training, p=p)
    pool5 = mpool_op(conv5_2, name="pool5", kh=2, kw=2, dw=2, dh=2)

    # phase 6
    conv6 = conv_op(pool5, name="conv6", kh=3, kw=3, n_out=256, dh=1, dw=1, is_training=is_training, p=p)

    # phase 7: flatten pool4
    shp = conv6.get_shape()
    flattened_shape = shp[1].value * shp[2].value * shp[3].value
    reshp1 = tf.reshape(conv6, [-1, flattened_shape], name="reshp1")
    conv6_drop = tf.nn.dropout(reshp1, 0.7, name="conv6_drop")

    # phase 7: feature layer
    fc7 = fc_op(conv6_drop, n_out=NUM_FEATURE, name="fc7", p=p)
    fc7_drop = tf.nn.dropout(fc7, 0.5, name="fc7_drop")

    # phase 8: network output
    fc8 = fc_op(fc7_drop, n_out=NUM_CLASSES, name="fc8", p=p)
    logits = fc8
    feature = fc7
